...
/*   Updated: 2018/10/24 22:00:59 by gwood            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "graphics.h"

static void	kt_drawtri3d_up(t_drawtri_tex *t, t_vec3d p0, t_vec3d p1, t_vec3d p2)
{
	int				x;
	int				y;
	...
	while (++y < t->y_end)
	{
		...
		x = t->x_start - 1;
		while (++x < t->x_end)
			XDrawPoint(t->x->dpy, t->x->win, t->x->gc, x, y);
	}
}

static void	kt_drawtri3d_down(t_drawtri_tex *t, t_vec3d p0, t_vec3d p1, t_vec3d p2)
{
	int				x;
	int				y;
...
void	kt_drawtri3d_tex(t_xvars *x, t_vert verts[3], )
{
	double			alpha_split;
	t_vec3d			vi;
	t_drawtri_tex	t;
	...
	else
	{
		...
		vi.y = p0.y + (p2.y - p0.y) * alpha_split;
		if (p1.x < vi.x)
		{
