...
/*   Updated: 2018/10/21 10:53:56 by gwood            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			kt_drawtri2d(t_xvars *d, t_vec2d p0, t_vec2d p1, t_vec2d p2)
{
	...
	if (p0.y == p1.y)
	{
		if (p1.x < p0.x)
			kt_vec2d_swap(&p0, &p1);
		kt_drawtri2d_down(d, p0, p1, p2);
	}
	else if (p1.y == p2.y)
	{
		if (p2.x < p1.x)
			kt_vec2d_swap(&p1, &p2);
		kt_drawtri2d_up(d, p0, p1, p2);
	}
	else
	{
		...
		if (p1.x < vi.x)
		{
			kt_drawtri2d_up(d, p0, p1, vi);
			kt_drawtri2d_down(d, p1, vi, p2);
		}
		else
		{
			kt_drawtri2d_up(d, p0, vi, p1);
			kt_drawtri2d_down(d, vi, p1, p2);
		}
	}
}
